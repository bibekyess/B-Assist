// Built on top of this awesome resource
// https://github.com/BruceMacD/chatd/blob/a2160988a7f08ca682cbe4b07345868f7c1bc2d3/src/service/ollama/ollama.js
...
class OllamaOrchestrator {
  ...
  static async getOllama() {
    if (this.instance === null) {
      const ollamaModule = await import("ollama/browser");
  ...
  async serve() {
    ...
    switch (process.platform) {
      case "win32":
        exe = "ollama.exe";
        appDataPath = path.join(os.homedir(), "AppData", "Local", "bassist");
        break;
      case "darwin":
        exe = "ollama-darwin";
      ...
      case "linux":
        ...
        appDataPath = path.join(os.homedir(), ".config", "bassist");
        break;
      default:
    ...
    const pathToBinary = path.join(__dirname, "runner", exe);
  ...
  async run(model, fn) {
    try {
      ...
      logInfo('bassist is running offline, failed to pull');
    }
    // load the model
  ...
  async chat(model, prompt, fn) {
    ...
    try {
      logDebug(`input to llm: ${this.messages[0]}`);
    ...
    // this.messages.push(assistant); # For now, I don't need to remember old conversations
  }
